...
function Books() {
  ...
  return (
    <div>
      ...
      <p className='text-center italic text-[var(--primaryColor)] mb-1'>
        {"("}The things that belong to our peace{")"}
      </p>
      <p className='text-center md:w-[50%] m-auto italic text-[var(--primaryText)] px-1 text-sm md:text-base'>
      ...
      <p className='text-right md:w-[50%] m-auto italic text-[var(--primaryText)] px-1 mb-2 font-semibold text-sm md:text-base'>
        -1 Timothy 1:18 (GNT)
      </p>
      {isLoading ? (
        <Loading />
          ...
          {data.map((book: BookType) => {
            return (
              ...
              >
                ...
                <Link
                  to={`/books/${book.year}`}
                  className='uppercase text-sm md:text-base hover:text-[var(--hoverColor)]'
                >
                  {book.year} "{book.word}"
                </Link>
                <button onClick={() => mutate(Number(book.year))}>
                  <FaTrash />
                </button>
              </div>
            )
